...
| `scripts/detect-as-native-evidence-before-verdict.sh` | runtime-loaded deterministic detector | AS-57 docs-readback-only adoption/readiness/fallback verdict detector added by issue #171 | Keep tracked as an existing deterministic detector. |
| `scripts/detect-as-instruction-contradiction.sh` | runtime-loaded deterministic detector | AS-58 instruction-contradiction detector (cited-live-while-dead / mandate-and-forbid) added by issue #194; archive/log false-positive class fixed by issue #200 | Keep tracked; **n=1 keep-candidate, NOT graduated.** n=2 hunt (issue #198) vs BMA `main` previously FIRED (6 contradictions) but was manually verified **FALSE POSITIVE**: archive surfaces and historical learning rows were being treated as live instruction mandates. Issue #200 fixed that class by excluding `docs/archive/`, `docs/handoffs/`, and `docs/completions/`, filtering non-action tokens, and keeping historical `LEARNINGS.md` rows out of current mandate/forbid scoring. BMA rerun at `7e20f549` is `fired:false`; true-positive fixtures still fire. See LEARNINGS L29-L30. |
| `scripts/detect-as-assimilation-github-work-management-gap.sh` | runtime-loaded deterministic detector | AS-59 assimilation GitHub work-management detector for `ASSIMILATION_GITHUB_WORK_MANAGEMENT_V1` field gaps and closure/automation/mutation overclaims added by issue #205 | Keep tracked as an existing deterministic detector; permission-insufficient readback is access/owner routing, not repo-quality failure. |
...
| `scripts/detect-rework-recurrence.sh` | runtime-loaded deterministic detector (DS-49) | git-observable re-work recurrence detector added by this PR | Keep tracked; live-checkout only, MEDIUM/T2 **n=1 keep-candidate** (severity/tier + graduation operator-gated). n=2 hunt (issue #198) vs BMA `main` @`9b6aed4c`: **correct SILENCE** (fired:false, no-finalize-commits — 0 subject-line finalize acts / `Closes #N` in 100 commits; BMA closes GitHub-native via `Refs #N` + PR-merge suffix). No-FP data point but no-anchor (never reached rework stage) + honest recall caveat: subject-anchored finalize classifier is blind to GitHub-native closure. Silence != graduation → stays n=1. Complements `briancl2/repo-agent-core#103`. **Recall boundary DECIDED = ACCEPT (issue #202):** extending the finalize classifier to treat GitHub-native closure (`(#N)` PR-merge suffix / `Refs #N`) as a finalize anchor was empirically rejected — a simulated extended classifier run read-only against repo-auditor's OWN clean history, **measured at parent commit `c92d9d7`** (pre-this-docs-commit history), flipped `finalize_commit_count` 1→43 (every squash-merge commit becomes a "finalize") and `fired:true` with 7 recurring areas / 14 hits, dominated by shared-infra false positives ("Add AS-49 (#156)" → "Fix **AS-50** FPs (#160)" on `as_signature_scan.py`; "Add AS-51 (#167)" → "Fix **Phase-0** bugs (#186)" on `check.sh`) — independent detector work re-touching shared files, NOT rework of the same declared-done unit. The 1→43 delta is anchored to `c92d9d7`: at the PR head this docs commit's own `Closes #202` subject raises baseline `finalize_commit_count` to 2, so reproduce via `git worktree add --detach <tmp> c92d9d7 && bash scripts/detect-rework-recurrence.sh <tmp>` (=1); the =43 is the same run with `(#N)`+`Refs #N` added to FINALIZE_PATTERN. The `(#N)` suffix is merge plumbing (no author doneness declaration); `Refs #N` is a non-closing reference. Structural boundary: reliable GitHub-native closure detection needs PR-body/merge-state (gh API), a non-git-observable architecture out of scope for a git-log detector. Known limitation, not a defect: DS-49 under-recalls on GitHub-native-closure repos and stays n=1. See LEARNINGS L29, L31. |
