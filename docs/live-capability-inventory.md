...
| `scripts/detect-as-instruction-contradiction.sh` | runtime-loaded deterministic detector | AS-58 instruction-contradiction detector (cited-live-while-dead / mandate-and-forbid) added by issue #194 | Keep tracked; **n=1 keep-candidate, NOT graduated.** n=2 hunt (issue #198) vs BMA `main` @`9b6aed4c` FIRED (6 contradictions) but manually verified **FALSE POSITIVE**: both cited-live-while-dead pairs treat `docs/archive/**` files as live surfaces and historical `LEARNINGS.md` log mentions as live citations, on tokens (PRINCIPLE_LEDGER / OPERATING_MODEL_SCORECARD) BMA correctly archived (F1/#1284); mandate/forbid axis emitted garbage tokens and mis-scored "recommended-not-gated" `make review`. FP class routed to owner (do NOT fix in-scope). See LEARNINGS L29. |
