...
- **Detects:** Route-changing learning/failure material that lacks owner-surface evidence, advisory memory disposition, exact readback/no-capture discipline, fallback, owner action, literal-safe comment readback, or foreground-only boundaries.
- **Signal:** A route-changing learning/failure receipt, `Learning / Recovery` block, foreground Hermes failure route, GBrain exact-handle replay note, or literal-bearing GitHub readback surface omits GitHub/raw evidence, omits GBrain slug or `no_capture_reason`, omits exact readback or no-capture discipline for a GBrain slug, omits fallback without memory, omits owner action, lacks literal-safe GitHub comment/readback evidence, treats broad GBrain search miss as absence without exact-handle replay, overclaims stale/contradictory/failed GBrain evidence, or claims background GBrain/Hermes/controller/scheduler/queue/daemon/retry-loop ownership.
...
- **Detects:** Hermes foreground doer/checker reliability material that omits eligibility, launcher/failure-guidance evidence, coordinator review, validation owner, publication scope, promotion/demotion evidence, checker disposition, or foreground-only forbidden-authority boundaries.
- **Signal:** A Hermes foreground reliability, Hermes doer, Hermes checker, or checker-shadow surface omits Hermes eligibility, attempt role, launcher/run receipt, failure guidance before Codex fallback or clean-success reason, coordinator review, validation owner, publication scope, promotion gate, demotion/rejection trigger, checker-shadow disposition, or bounded non-claims; fills those fields with vague placeholders; sets Hermes as validation owner; grants checker-shadow approval/edit ownership; publishes branches or PRs without scoped authority; claims autonomous retry or unscoped fix-cycle behavior; or describes Hermes as primary operator, coordinator, validator, merger, CI owner, retry loop, scheduler, queue, daemon, controller, background worker, automatic issue/PR creator, Campaign Sync owner, recovery owner, downstream mutator, or `hermes -z` adoption path.
...
- **Check:** Scan owner evidence text for Hermes reliability/doer/checker surfaces and require portable reliability fields plus foreground-only bounded non-claims, including publication-scope and checker-shadow no-edit/no-approval disposition. Suppress detector docs/templates/tests/fixtures and the shared repo-agent-core Hermes foreground reliability contract/template.
