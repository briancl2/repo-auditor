#!/usr/bin/env bash
# Verify bounded AS-20..AS-38 replay keeps direct closure and clean recovery runtime clean.
set -euo pipefail
...
assert from_file["signature_ids"] == [f"AS-{index}" for index in (*range(20, 37), 38)]
...
assert any("retained AS-20 through AS-38" in claim for claim in from_file["bounded_non_claims"])
PY
