...
role_rule='EXTERNAL_CRITIQUE_CAPABILITY Version: 1.0 context support; panel/latest-panel requires named high-stakes context before invocation; finding quota: none; no-finding results are valid; blocker findings and advisory findings are separated; pass budget one initial pass plus one follow-up pass with a stopping condition; authority refs AGENTS.md and GitHub issue/PR/check/merge truth; privacy/redaction boundaries; fleet findings advisory until owner evidence exists.'
...
python3 - "$REPO_ROOT" "$MISSING_REPO" "$CLEAN_REPO" "$DRIFT_REPO" "$ROLE_REPO" <<'PY'
import json
import subprocess
import sys

repo_root, missing_repo, clean_repo, drift_repo, role_repo = sys.argv[1:6]
...
assert clean["signals"]["contract_source_version"] == "1.0", clean
