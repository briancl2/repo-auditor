...
PASS=0
FAIL=0
TEST_ROOT="$REPO_ROOT/work/test-work-close-github-native-$$"
...
make_fixture() {
    ...
    cat > "$work_dir/pre-audit/SCORECARD.json" <<'EOF'
{
  "schema_version": "test",
  "composite": 55
}
...
WORK1="$TEST_ROOT/work-github-native"
make_fixture "$WORK1"
WORK_CLOSE_OPS_LEDGER="$TEST_ROOT/ops.jsonl" \
    bash scripts/work-close.sh "$WORK1" \
    --no-novel-findings "No reusable repo-auditor learning beyond the closure-authority change under test." \
...
ops_mode=$(python3 -c "import json; print(json.load(open('$TEST_ROOT/ops.jsonl'))['data']['score_session_mode'])")
...
WORK2="$TEST_ROOT/work-short-rationale"
make_fixture "$WORK2"
set +e
WORK_CLOSE_OPS_LEDGER="$TEST_ROOT/ops-short.jsonl" \
    bash scripts/work-close.sh "$WORK2" --github-native-closeout "short" > "$TEST_ROOT/short.out" 2>&1
short_status=$?
set -e
...
WORK3="$TEST_ROOT/work-default"
make_fixture "$WORK3"
WORK_CLOSE_OPS_LEDGER="$TEST_ROOT/ops-default.jsonl" \
    bash scripts/work-close.sh "$WORK3" \
    --no-novel-findings "No reusable repo-auditor learning beyond proving the default score-session path stays active."
...
default_mode=$(python3 -c "import json; print(json.load(open('$TEST_ROOT/ops-default.jsonl'))['data']['score_session_mode'])")
